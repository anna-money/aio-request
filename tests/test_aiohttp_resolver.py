...
async def test_multiple_resolve():
    ...
    try:
        first = await resolver.resolve("host", 80)
        second = await resolver.resolve("host", 80)
    finally:
        await resolver.close()

    base_resolver.resolve.assert_called_once_with("host", 80)
...
async def test_resolve_failures_in_background():
    ...
    try:
        ...
        second = resolver.resolve_no_wait("host", 80)
    finally:
        await resolver.close()

    base_resolver.resolve.assert_called_with("host", 80)
