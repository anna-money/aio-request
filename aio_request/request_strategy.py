...
def sequential_strategy(
    *, attempts_count: int = 3, delays_provider: collections.abc.Callable[[int], float] = linear_backoff_delays()
) -> RequestStrategy:
...
def retry_until_deadline_expired(
    strategy: RequestStrategy, *, delays_provider: collections.abc.Callable[[int], float] = linear_backoff_delays()
) -> RequestStrategy:
