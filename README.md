...
from aio_request import MethodBasedStrategy, RequestStrategiesFactory, get, post
...
async with client_session:
    ...
    async with request_strategy.request(get("/thing"), deadline=5) as response:
        pass  # process response here

    async with request_strategy.request(post("/thing"), deadline=5) as response:
        pass  # process response here
```